        ...
        <div className="h-10 w-10 rounded-full bg-gray-700 overflow-hidden flex-shrink-0">
          {tweet.author?.avatar ? (
            <img
              ...
              alt={tweet.author?.displayName || tweet.author?.username || "User"}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1">
            <span className="font-semibold">
              {tweet.author?.displayName || tweet.author?.username || "User"}
            </span>
            <span className="text-gray-500">@{tweet.author?.username || "user"}</span>
            <span className="text-gray-500">·</span>
            <span className="text-gray-500">{tweet.timestamp || "now"}</span>
          </div>
          <p className="mt-1 whitespace-pre-wrap text-sm text-gray-200">{tweet.content}</p>
          {tweet.image ? (
            <img src={tweet.image} alt="" className="mt-3 rounded-2xl max-h-80 w-full object-cover" />
          ) : null}
        </div>
      </div>
    </div>
