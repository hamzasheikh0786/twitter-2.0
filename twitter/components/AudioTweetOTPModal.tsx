...
interface AudioTweetOTPModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSuccess: () => void;
    email: string;
...
export default function AudioTweetOTPModal({ 
    isOpen, 
    onClose, 
    onSuccess, 
    email 
}: AudioTweetOTPModalProps) {
    ...
    const handleVerify = async () => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/audio-tweet/verify-otp`, {
                method: 'POST',
    ...
    const handleResend = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/audio-tweet/request-otp`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                ...
                <CardHeader className="relative pb-6">
                    ...
                    <div className="text-center">
                        <div className="mb-6 flex justify-center">
                            <TwitterLogo size="xl" className="text-purple-500" />
                        </div>
                            ...
                            <div className="flex gap-3 justify-center" role="group" aria-label="OTP code">
                                {otp.map((digit, index) => (
                                    <input
                                        ...
                                        onPaste={handlePaste}
                                        className="w-12 h-14 text-center text-2xl font-bold bg-transparent border-gray-600 text-white placeholder-gray-400 focus:border-purple-500 focus:outline-none"
                                        disabled={isLoading}
